...
const EditCar = ({car}) => {
  ...
  const handleRemoveCurrentImage = (index) => {
    setCurrentImagesToDelete((prevFiles) => [...prevFiles, currentImages[index]]);
  ...
  return (
    <div className={styles.box}>
      ...
      <div className={styles.status_menu}>
        ...
        <textarea ref={carBrandRef} className={styles.status_textarea} defaultValue={car.make_model} placeholder="For example - Honda Prelude / BMW E39"/>
      </div>
      ...
      <div className={styles.status_menu}>
        ...
        <textarea ref={carYearRef} className={styles.status_textarea} defaultValue={car.year} placeholder="For example - 1998"/>
      </div>
      ...
      <div className={styles.status_menu}>
        ...
        <textarea ref={carDescriptionRef} className={styles.status_textarea_desc} defaultValue={car.desc_car} placeholder="Write what you want.. Modifications etc."/>
      </div>
      ...
      <div className={styles.image_box}>
        {currentImages.map((file, index) => (
          <div key={index} className={styles.image_container}>
            <img className={styles.image} src={file.car_image_url} alt={`Added image ${index + 1}`} />
            <button className={styles.remove_button} onClick={() => handleRemoveCurrentImage(index)}>
              <FaTrash />
            </button>
          </div>
        ))}
      ...
      <div className={styles.image_box}>
        {selectedFiles.map((file, index) => (
          <div key={index} className={styles.image_container}>
            <img className={styles.image} src={URL.createObjectURL(file)} alt={`Added image ${index + 1}`} />
            <button className={styles.remove_button} onClick={() => handleRemoveImage(index)}>
              <FaTrash />
            </button>
          </div>
        ))}
      ...
      <div className={styles.post_delte_bar}>
        ...
        <div className={styles.status_main}>
          <button className={styles.status_delete} onClick={() => handleDeleteCar()}><b>Are you sure?</b></button>
        </div>
        :
        <div className={styles.status_main}>
          <button className={styles.status_delete} onClick={() => setConfirmDelete(true)}>Delete Car</button>
        </div>}
        
        <div className={styles.status_main}>
          {isLoading ? <button className={styles.status_loading}>Loading</button>
          : <button className={styles.status_save} onClick={() => handleSubmit()}>Save</button>}
        </div>
      </div>
